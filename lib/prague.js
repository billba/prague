...
var Do = /** @class */ (function (_super) {
    ...
    Do.prototype.do$ = function () {
        return rxjs_1.of(this.action).pipe(operators_1.map(function (action) { return action(); }), operators_1.flatMap(exports.toObservable), operators_1.mapTo(true));
    };
    return Do;
}(Route));
exports.Do = Do;
...
var Router = /** @class */ (function () {
    function Router(router) {
        if (router == null)
            ...
            this.route$ = function () {
                ...
                return rxjs_1.of(router).pipe(operators_1.map(function (router) { return router.apply(void 0, args); }), operators_1.flatMap(exports.toObservable), operators_1.flatMap(function (result) { return rxjs_1.of(Router.normalizedRoute(result)); }));
            };
        else
    ...
    Router.prototype.do$ = function () {
        ...
        return this
            .route$.apply(this, args).pipe(operators_1.flatMap(function (route) { return route.do$(); }));
    };
    Router.prototype.do = function () {
        var args = [];
        for (var _i = 0; _i < arguments.length; _i++) {
    ...
    Router.prototype.map = function (mapRoute) {
        ...
        return new Router(function () {
            ...
            return _this
                .route$.apply(_this, args).pipe(operators_1.flatMap(function (route) { return Router.from(mapRoute).route$(route); }));
        });
    };
    Router.prototype.mapByType = function (mapTypeToRouter) {
        return this.map(function (route) { return route
            .type$()
            .pipe(operators_1.map(function (type) { return mapTypeToRouter[type]; }), // workaround for TypeScript bug
        operators_1.filter(function (router) { return !!router; }), operators_1.take(1), operators_1.flatMap(function (router) { return Router.from(router).route$(route); }), operators_1.defaultIfEmpty(route)); });
    };
    ...
    Router.prototype.tap = function (fn) {
        return this.map(function (route) { return exports.toObservable(fn(route)).pipe(operators_1.mapTo(route)); });
    };
    ...
    Router.prototype.beforeDo = function (action) {
        return this
            .tap(exports.doable)
            .mapByType({
            do: _do(function (route) { return exports.toObservable(action()).pipe(operators_1.flatMap(function (_) { return route.do$(); })); })
        });
    };
    Router.prototype.afterDo = function (action) {
        return this
            ...
            do: _do(function (route) { return route
                .do$()
                .pipe(operators_1.flatMap(function (_) { return exports.toObservable(action()); })); })
        });
    };
    return Router;
}());
...
function first() {
    ...
    return Router.from(function () {
        ...
        return rxjs_1.from(routers).pipe(
        // we put concatMap here because it forces everything after it to execute serially
        operators_1.concatMap(function (router) {
            var _a;
            return (_a = Router
                .from(router)).route$.apply(_a, args);
        }), operators_1.filter(function (route) {
            if (route instanceof NamedAction || route instanceof Do)
                return true;
            if (route instanceof No)
                return false;
            throw firstError;
        }), operators_1.take(1), // so that we don't keep going through routers after we find one that matches
        operators_1.defaultIfEmpty(No.default));
    });
}
exports.first = first;
...
function best() {
    ...
    return Router.from(function () {
        ...
        return rxjs_1.from(routers).pipe(operators_1.flatMap(function (router) {
            var _a;
            return (_a = Router
                .from(router)).route$.apply(_a, args);
        }), operators_1.flatMap(function (route) {
            if (route instanceof No)
                return rxjs_1.empty();
            if (route instanceof NamedAction)
                return rxjs_1.of(route);
            if (route instanceof Multiple)
                return rxjs_1.from(route.routes);
            throw bestError;
        }), operators_1.toArray(), operators_1.flatMap(function (routes) { return rxjs_1.from(routes.sort(function (a, b) { return b.score - a.score; })).pipe(operators_1.takeWhile(function (route) { return route.score + tolerance >= routes[0].score; }), operators_1.toArray(), operators_1.map(function (routes) {
            if (routes.length === 0)
                return No.default;
            if (routes.length === 1)
                return routes[0];
